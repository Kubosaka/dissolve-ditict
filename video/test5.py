...
for i in range(1,len(image_arrays)):
    ...
    if i>1:
        ...
        print(r_img)
        count = np.count_nonzero((0.5<r_img)&(r_img<1.5))
...
#     if (r_imgs[i+1]-r_imgs[i])*(r_imgs[i]-r_imgs[i-1])<0:
#         print("({0:.2f}-{1:.2f})*({1:.2f}-{2:.2f})={3:.2f}".format(r_imgs[i+1],r_imgs[i],r_imgs[i-1],(r_imgs[i+1]-r_imgs[i])*(r_imgs[i]-r_imgs[i-1])))
#         if (r_imgs[i]-r_imgs[i-1])>0:
#             print("極大値は{0}です。".format(i))
#         else:
#             print("極小値は{0}です。".format(i))
